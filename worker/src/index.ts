...
async function handleHttpProxy(
  ...
  if (contentType.includes("text/css")) {
    ...
    const body = text.length > MAX_TEXT_REWRITE_BYTES ? text : rewriteCssText(text, targetUrl.toString());
  ...
  if (contentType.includes("javascript")) {
    ...
    // (fetch()/KV/DO waits don't count, but this string processing does) —
    // a limit that is NOT enforced in local `wrangler dev`, so this can
    // pass every local test and still fail in production specifically on
    // large bundles (YouTube's JS chunks routinely run several hundred KB
    // to multiple MB). Past this size, skip rewriting and pass the text
    // through unmodified rather than risking a hard CPU-limit error —
    // some embedded URLs in huge files may not get proxied, but the
    // script loads instead of the request failing outright.
    const rewritten =
      text.length > MAX_TEXT_REWRITE_BYTES ? text : rewriteJsUrls(text, targetUrl.toString());
...
// Cloudflare Workers Free gives 10ms CPU time per request; this threshold
// is a pragmatic guess at where full-buffer regex rewriting risks eating
// too much of that budget on a single response. Workers Paid (30s+ CPU
// time by default) can raise or remove this without much risk — see
// README for how to adjust it.
const MAX_TEXT_REWRITE_BYTES = 300_000;
